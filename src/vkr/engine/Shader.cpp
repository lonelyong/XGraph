...
unsigned int createShader(const char* code, int type) {
...
VI_OBJECT_META_IMPL(Shader, VKObject);
...
void Shader::use(State& state) {
    if (!isCreated(state)) VKObject::create(state);
...
template <typename T> void Shader::set(State& state, int loc, const T& val) {
...
int Shader::onCreate(State& state) {
