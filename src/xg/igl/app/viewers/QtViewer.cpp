...
class GraphicContextQt : public GraphicContext {
  ...
  protected:
    virtual GLfuncs* createGLfuncs() override
    {
        ...
        return GLfuncs::loadGLLoader((GLfuncs::Loader)[](const char* name) { return (void*)ctx->getProcAddress(name); });
    }
...
void QtViewer::initializeGL()
{
    ...
    funcs->oglEnable(GL_DEPTH_TEST);
    // glEnable(GL_TEXTURE_2D); //removed after v3.2+
    funcs->oglEnable(GL_PROGRAM_POINT_SIZE);
    // glEnable(GL_CULL_FACE);
    // glCullFace(GL_BACK);

    funcs->oglFrontFace(GL_CCW);
    funcs->oglDepthFunc(GL_LESS);
...
void QtViewer::mousePressEvent(QMouseEvent* event)
{
    ...
    if (event->button() == Qt::LeftButton) { btn = BUTTON_LEFT; }
