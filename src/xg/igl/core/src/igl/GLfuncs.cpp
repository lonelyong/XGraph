...
#if !defined(__APPLE__) && !defined(__HAIKU__)
    if (s_iglGetProcAddressPtr != NULL) { result = s_iglGetProcAddressPtr(namez); }
#endif
    if (result == NULL) {
#if defined(_WIN32) || defined(__CYGWIN__)
...
#pragma endregion
    ...
    if (major >= 11) {
        GLint_t nb_exts = 0;
        funcs->oglGetIntegerv(IGL_NUM_EXTENSIONS, &nb_exts);
        ...
        auto exts = (const char*)funcs->oglGetString(IGL_EXTENSIONS);
        if (exts) {
