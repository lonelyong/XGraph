...
{
    if (!d->ctx || d->is_freezed) { return -1; }

    if (d->is_first_frame) { d->is_first_frame = false; }
    ...
    if (d->use_master_view_matrix) { cam->setViewMatrix(master_cam->getViewMatrix()); }

    if (d->use_master_proj_matrix) { cam->setProjectionMatrix(master_cam->getProjectionMatrix()); }
...
bool Renderer::handleEvent(Event* e)
{
    ...
    if (false == handled) {
        if (d->cm.hasValue()) { handled |= d->cm->handleEvent(e); }
    }

    if (false == handled) {
        if (d->scene.hasValue()) { handled |= d->scene->handleEvent(e); }
    }
...
void Renderer::update(UpdateContext* ctx)
{
    ...
    if (d->scene.hasValue()) { d->scene->update(ctx); }
}
