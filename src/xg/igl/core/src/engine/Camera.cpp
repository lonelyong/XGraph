...
void Camera::applyViewport(State& state) const
{
    ...
    funcs->oglViewport(vp_x_, vp_y_, vp_w_, vp_h_);
    funcs->oglScissor(vp_x_, vp_y_, vp_w_, vp_h_);
    funcs->oglEnable(IGL_SCISSOR_TEST);
}

void Camera::applyAllExceptViewport(State& state) const
{
    auto funcs = state.getContext()->getFuncs();
    funcs->oglClearColor(clear_color_.r, clear_color_.g, clear_color_.b, clear_color_.a);
    funcs->oglClearDepth(clear_depth_);
    funcs->oglClearStencil(clear_stencil_);
    funcs->oglClear(clear_mask_);
}
