...
{
    auto funcs = state.getContext()->getFuncs();
    funcs->oglBindBuffer(getTarget(), getId(state));
    return true;
}
...
bool BufferObject::onUnbind(State& state)
{
    ...
    funcs->oglGetIntegerv(getBindingOfTarget(getTarget()), &curr_id);

    if (curr_id == getId(state)) { funcs->oglBindBuffer(getTarget(), 0); }
