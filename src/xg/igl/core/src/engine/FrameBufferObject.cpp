...
{
    if (getNumInstances()) { throw vine::Exception(vine::Exception::INVALID_OPERATION); }
...
{
    auto iter = d->components.find(comp);
    if (iter == d->components.end()) { return nullptr; }
...
GLuint_t FrameBufferObject::onCreate(State& state)
{
    ...
    GLuint_t id;
    funcs->oglGenFramebuffers(1, &id);
    funcs->oglBindFramebuffer(IGL_FRAMEBUFFER, id);

    for (auto&& kv : d->components) {
        auto& buffer = kv.second;
        buffer->bind(state);
        if (buffer->isKindOf<Texture2D>()) { funcs->oglFramebufferTexture2D(IGL_FRAMEBUFFER, kv.first, IGL_TEXTURE_2D, buffer->getId(state), 0); }
        else if (buffer->isKindOf<RenderBuffer>()) {
            funcs->oglFramebufferRenderbuffer(IGL_FRAMEBUFFER, kv.first, IGL_RENDERBUFFER, buffer->getId(state));
        }
        buffer->unbind(state);
    }

    if (funcs->oglCheckFramebufferStatus(IGL_FRAMEBUFFER) != IGL_FRAMEBUFFER_COMPLETE) {
...
bool FrameBufferObject::onRelease(State& state)
{
    auto id    = getId(state);
    auto funcs = state.getContext()->getFuncs();
    funcs->oglDeleteFramebuffers(1, &id);
    return true;
}
...
bool FrameBufferObject::onUnbind(State& state)
{
    GLint_t curr_id = 0;
    auto    funcs   = state.getContext()->getFuncs();
    funcs->oglGetIntegerv(IGL_FRAMEBUFFER_BINDING, &curr_id);
    if (curr_id == getId(state)) { funcs->oglBindFramebuffer(IGL_FRAMEBUFFER, 0); }
    return true;
}
