...
template <typename T>
Array<T>::Type Array<T>::getType() const
{
    if constexpr (std::is_same<T, int8_t>::value) { return ARRAY_INT8; }
...
template <typename T>
BufferObject::Target Array<T>::getTarget() const
{ return ARRAY_BUFFER; }

template <typename T>
BufferObject::Usage Array<T>::getUsage() const
{ return STATIC_DRAW; }

template <typename T>
GLsizei_t Array<T>::size() const
{ return d->impl.size(); }
...
template <typename T>
void* Array<T>::data_ptr()
{ return d->impl.data(); }

template <typename T>
const void* Array<T>::data_ptr() const
{ return d->impl.data(); }

template <typename T>
T& Array<T>::at(GLsizei_t index)
{ return d->impl[index]; }

template <typename T>
const T& Array<T>::at(GLsizei_t index) const
{ return d->impl[index]; }

template <typename T>
void* Array<T>::valueAt(GLsizei_t index)
{ return &d->impl.at(index); }

template <typename T>
void Array<T>::resize(GLsizei_t size)
{ d->impl.resize(size); }

template <typename T>
void Array<T>::reserve(GLsizei_t size)
{ d->impl.reserve(size); }
...
template <typename T>
GLsizei_t Array<T>::capacity() const
...
template <typename T>
GLsizei_t Array<T>::sizeOfItem() const
{ return sizeof(item_type); }

template <typename T>
GLuint_t Array<T>::onCreate(State& state)
{
    auto funcs = state.getContext()->getFuncs();
    auto id    = ArrayBuffer::onCreate(state);
    if (id != IGL_ZERO) {
        funcs->oglBindBuffer(getTarget(), id);
        funcs->oglBufferData(getTarget(), size() * sizeof(item_type), (void*)data(), getUsage());
        funcs->oglBindBuffer(getTarget(), 0);
    }
    return id;
}

template <typename T>
bool Array<T>::onUpdate(State& state)
{ return true; }
...
template <typename T>
Array<T>::const_iterator Array<T>::cbegin() const
...
template <typename T>
Array<T>::const_iterator Array<T>::cend() const
...
template <typename T>
void Array<T>::iterator::operator++(difference_type i)
{ pos_ += i; }
...
template <typename T>
Array<T>::iterator Array<T>::iterator::operator+(difference_type d) const
...
template <typename T>
Array<T>::iterator& Array<T>::iterator::operator+=(difference_type d)
{
    pos_ += d;
    return *this;
}
...
template <typename T>
bool Array<T>::iterator::operator!=(const Array<T>::iterator& it) const
{ return !(*this == it); }
...
template <typename T>
void Array<T>::const_iterator::operator++()
{ ++pos_; }

template <typename T>
void Array<T>::const_iterator::operator++(int i)
{ pos_ += i; }
...
template <typename T>
bool Array<T>::const_iterator::operator!=(const Array<T>::const_iterator& it) const
{ return !(*this == it); }
