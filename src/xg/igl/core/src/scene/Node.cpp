...
#include <xg/igl/scene/Node_p.hpp>
...
{
    if (!d->state_set.get()) { d->state_set = new StateSet(); }
...
{
    if (d->bound_is_dirty) { onComputeBoundingBox(d->bb); }
    return d->bb;
}
...
void Node::dirtyBound()
{
    ...
    for (int i = 0; i < nparents; ++i) { getParentAt(i)->dirtyBound(); }
}
