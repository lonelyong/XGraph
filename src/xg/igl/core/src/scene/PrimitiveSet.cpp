#include <xg/igl/scene/PrimitiveSet.hpp>
...
void DrawArrays::draw(State& state)
{
    ...
    if (insts > 1) { funcs->oglDrawArraysInstanced(mode, offset_, count_, insts); }
    else {
        funcs->oglDrawArrays(mode, offset_, count_);
    }
}
...
void DrawElementsUInt::draw(State& state)
{
    ...
    auto mode  = getMode();
    auto insts = getInstances();
    if (insts > 1) { funcs->oglDrawElementsInstanced(mode, indices_.size(), IGL_UNSIGNED_INT, (void*)indices_.data(), insts); }
    else {
        funcs->oglDrawElements(mode, indices_.size(), IGL_UNSIGNED_INT, (void*)indices_.data());
    }
}
