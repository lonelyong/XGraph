    ...
    if (!d->vao) {
        ...
        for (auto& kv : vbos) {
            ...
            if (arr->size() > 1) {
                ...
                funcs->oglVertexAttribPointer(loc, size_of_item / sizeof(GLfloat_t), IGL_FLOAT, IGL_FALSE, size_of_item, 0);
                funcs->oglEnableVertexAttribArray(loc);
                arr->unbind(state);
            }
            else {
