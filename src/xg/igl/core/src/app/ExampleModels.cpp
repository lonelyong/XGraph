...
#include <xg/igl/app/DefaultUniformStore.hpp>
...
Model* ExampleModels::createCube(float len, const Vec3d& posi, bool with_tex)
{
    ...
    Mat4d mat(1.0);
    if (with_tex) { mat = glm::rotate(mat, glm::radians(90.), Vec3d(1.0, 0., 0.)); }
    mat = glm::translate(mat, posi);
...
Model* ExampleModels::createPointCloud(int n)
{
    ...
    for (size_t i = 0; i < vertices->capacity(); i++) {
        ...
        colors->push_back({ rand() / static_cast<double>(INT16_MAX), rand() / static_cast<double>(INT16_MAX), rand() / static_cast<double>(INT16_MAX) });
    }
