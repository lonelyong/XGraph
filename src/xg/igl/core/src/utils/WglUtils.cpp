...
#    include <xg/igl/utils/WglUtils.hpp>
...
static bool wgl_proc_init()
{
    ...
    if (!SetPixelFormat(windowDC, pfi, &spfd)) { return false; }

    HGLRC glctx = wglCreateContext(windowDC);
    if (!wglMakeCurrent(windowDC, glctx)) { return false; }
...
{
    if (!wgl_proc_init()) { return nullptr; }

    int          pfi;
    unsigned int count;
    if (!wglChoosePixelFormatARB(dc, pixelAttrs, 0, 1, &pfi, &count)) { return NULL; }

    PIXELFORMATDESCRIPTOR spfd;
    DescribePixelFormat(dc, pfi, sizeof(spfd), &spfd);
    if (!SetPixelFormat(dc, pfi, &spfd)) { return NULL; }
    ...
    if (!wglMakeCurrent(dc, glctx)) { return NULL; }
