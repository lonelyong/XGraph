...
void aiProcessNode(Model* group, const aiScene* scene, aiNode* node)
{
    for (int i = 0; i < node->mNumMeshes; i++) {
        ...
        if (mesh->HasVertexColors(0)) {
            ...
            for (int j = 0; j < mesh->mNumVertices; j++) { colors->push_back(Vec4f(data[j].r, data[j].g, data[j].b, data[j].a)); }
        ...
        if (mesh->HasNormals()) {
            ...
            for (int j = 0; j < mesh->mNumVertices; j++) { norms->push_back(Vec3f(mesh->mNormals[j].x, mesh->mNormals[j].y, mesh->mNormals[j].z)); }
