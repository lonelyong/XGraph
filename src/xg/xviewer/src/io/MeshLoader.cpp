...
void aiProcessNode(osg::MatrixTransform** model, const aiScene* scene, aiNode* node)
{
    ...
    for (int i = 0; i < node->mNumMeshes; i++) {
        ...
        if (mesh->HasPositions()) {
            ...
            for (int j = 0; j < mesh->mNumVertices; j++) { vertices->push_back(osg::Vec3(mesh->mVertices[j].x, mesh->mVertices[j].y, mesh->mVertices[j].z)); }
        ...
        if (mesh->HasVertexColors(0)) {
            ...
            for (int j = 0; j < mesh->mNumVertices; j++) { colors->push_back(osg::Vec4(data[j].r, data[j].g, data[j].b, data[j].a)); }
        ...
        if (false && mesh->HasNormals()) {
            ...
            for (int j = 0; j < mesh->mNumVertices; j++) { norms->push_back(osg::Vec3(mesh->mNormals[j].x, mesh->mNormals[j].y, mesh->mNormals[j].z)); }
            geom->setNormalArray(norms, osg::Array::BIND_PER_VERTEX);
        }

        if (mesh->HasFaces()) {
            if (false && mesh->mNumFaces * 3 == mesh->mNumVertices) { geom->addPrimitiveSet(new osg::DrawArrays(GL_TRIANGLES, 0, mesh->mNumVertices)); }
            else {
    ...
    mt->addChild(geod);
    mt->setMatrix(osg::Matrix(m.a1, m.b1, m.c1, m.d1, m.a2, m.b2, m.c2, m.d2, m.a3, m.b3, m.c3, m.d3, m.a4, m.b4, m.c4, m.d4));
    for (int i = 0; i < node->mNumChildren; i++) {
        auto                  child_node  = node->mChildren[i];
        osg::MatrixTransform* child_model = nullptr;
        aiProcessNode(&child_model, scene, child_node);
        if (child_model) { mt->addChild(child_model); }
    }
    *model = mt;
}
...
bool MeshLoader::isSupported(const std::string& file)
{
    ...
    static std::set<std::string> exts     = { ".stl", ".obj", ".gltf", ".3mf", ".3ds", ".dxf", ".ifc", ".ac", ".ac3d", ".lxo", ".fbx", ".dae" };
